      ...
      case "classes": {
        ...
        return (
          <div className="grid gap-3 lg:grid-cols-[1fr_15rem]">
            {/* Brotato-style character grid: framed portraits, name under each */}
            <div className="grid gap-2">
              <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-white/55">
                {CLASS_KEYS.length} characters
              </p>
                ...
                {CLASS_KEYS.map((k) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group flex flex-col items-center gap-1 rounded-xl border-3 border-ink p-1 transition-transform ${
                        active
                          ? "bg-pop-tray outline outline-4 outline-gold"
                    ...
                    >
                      <span
                        ...
                        style={{
                          background: `radial-gradient(circle at 50% 42%, color-mix(in oklab, ${d.color} 34%, transparent), rgba(0,0,0,0.4) 74%)`,
                        }}
                      >
                        <ClassPortrait cls={k} skin={d.skin} className="h-full w-full" />
                      </span>
                      <span className="pop-label w-full truncate text-center text-[9px] font-black uppercase tracking-wide">
                        {d.name}
                      </span>
                    </button>
                  );
            ...
            {/* Detail card for the highlighted character */}
            <div className="pop-card flex flex-col gap-2 rounded-2xl p-3">
              <div
                className="grid h-28 w-full place-items-center overflow-hidden rounded-xl border-3 border-ink"
                style={{
                  background: `radial-gradient(circle at 50% 42%, color-mix(in oklab, ${def.color} 38%, transparent), rgba(0,0,0,0.4) 74%)`,
                }}
              >
                <ClassPortrait cls={cls} skin={def.skin} className="h-full w-full" />
              </div>
              <p className="pop-label text-center text-sm font-black uppercase tracking-wide">
                {def.name}
              </p>
              <p className="text-center text-[10px] leading-4 font-bold text-white/65">{def.blurb}</p>
              <div className="flex flex-col gap-1 rounded-xl bg-black/35 p-2">
                {def.buffs.map((b) => (
              ...
              <div className="flex items-center justify-between gap-2 text-[9px] font-black uppercase tracking-wide text-white/60">
                <span>Weapon</span>
                <span className="text-gold">{WEAPONS[def.weapon]?.name ?? def.weapon}</span>
              </div>
              <Price>
